...
constexpr int R = 4; // was 20
constexpr int MASK_SIZE = 2 * R + 1; // was 41, now 9
...
#ifndef __SYNTHESIS__
    std::printf("[kernel] detect_peaks() ENTER  num_windows=%d\n", num_windows);
#endif
...
#ifndef __SYNTHESIS__
    std::printf("[kernel] line_buf/window zero-init done\n");
#endif
...
#ifndef __SYNTHESIS__
        if (f < MAX_FREQ && num_windows > 0) {
            // only print rows that actually contain something interesting, to
            // avoid flooding stdout across ~2049 rows
            bool nonzero_row = false;
            for (int w = 0; w < num_windows; w++) {
                if (new_row[w] != 0.0f) { nonzero_row = true; break; }
            }
            if (nonzero_row) {
                std::printf("[kernel] row f=%d loaded, first few vals:", f);
                for (int w = 0; w < num_windows; w++) {
                    std::printf(" %.2f", new_row[w]);
                }
                std::printf("\n");
            }
        }
...
#ifndef __SYNTHESIS__
                        std::printf("[kernel] >>> peak #%d ACCEPTED at f=%d w=%d center_val=%.2f max_val=%.2f\n",
                                    pc, f_center, w_center, center_val, max_val);
#endif
                        pc++;
                    }
                }
...
#ifndef __SYNTHESIS__
    std::printf("[kernel] slide_window loop complete, pc=%d\n", pc);
#endif
...
#ifndef __SYNTHESIS__
    std::printf("[kernel] detect_peaks() EXIT   total peaks written=%d\n", pc);
#endif
}