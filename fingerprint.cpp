...
//    - cv::dft(...) is replaced with a hand-rolled iterative radix-2 FFT
//      (fft_radix2). WINDOW_SIZE=4096 is a power of two so radix-2 works
//      cleanly. This is the single biggest risk area: I have NOT validated
//      this FFT's numeric output against cv::dft on real audio. Before you
//      trust any hash coming out of this, run both versions on the same
//      test.mp3 and diff the peak lists / hashes.
//    - cv::dilate/cv::erode (used for 2D peak-picking) are replaced with a
//      direct diamond-neighborhood scan (see detect_peaks). The original
//      built its structuring element by dilating a single "on" pixel with a
//      3x3 cross, PEAK_NEIGHBORHOOD_SIZE (20) times. Iterating a cross
//      dilation N times produces exactly the set of points within Manhattan
//      (L1) distance N of the center — i.e. a diamond, NOT a square. I
//      reproduced that directly as `abs(df) + abs(dw) <= R` instead of
//      re-building a kernel image, which is simpler and avoids storing a
//      41x41 kernel Mat, but the *shape* of the neighborhood must stay a
//      diamond or peak-picking will disagree with the old build.
//
